...
def main() -> int:
    ...
    # apple-notes-brain layout: `data/` lives inside the Python package
    # under `src/` (bundled into the wheel via hatchling's
    # [tool.hatch.build.targets.wheel] packages=["src/apple_notes_brain"]).
    # obsidian-brain uses a top-level `data/` — only path difference between
    # the two repos. Same script body otherwise; both pull from the same
    # MTEB upstream so the JSONs are bit-identical given the same pin.
    out_file = repo_root / "src" / "apple_notes_brain" / "data" / "seed-models.json"
