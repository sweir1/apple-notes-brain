...
def _mock_state():
    """A minimal SemanticState stand-in for `_to_note_summary` calls
    that don't care about id translation — get_node will raise on the
    mock conn, falling through to the ZIDENTIFIER fallback path."""
    return MagicMock(conn=MagicMock())
...
def test_to_note_summary_carries_fused_score():
    ...
    summary = _to_note_summary(_mock_state(), r)
